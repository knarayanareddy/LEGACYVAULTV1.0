...
import {
  ...
  Settings, ChevronLeft, ChevronRight, Wallet, Lock, CheckCircle2, X
} from 'lucide-react';
  ...
  return (
    <aside className={`relative flex flex-col bg-gradient-to-b from-[#0d1225] to-[#0a0e1a] border-r border-vault-900/30 transition-all duration-300 h-full ${collapsed ? 'w-[72px]' : 'w-[260px]'}`}>
      <div className="flex items-center gap-3 px-5 py-6 border-b border-vault-900/30">
        ...
        {!collapsed && (
          <div className="animate-fade-in">
            <h1 className="text-lg font-bold text-white tracking-tight">LegacyVault</h1>
      ...
      {!collapsed && (
        <div className="mx-3 mb-4 p-4 rounded-2xl glass-card animate-fade-in">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-emerald-400 uppercase tracking-wider">Vault: {vaultStatus}</span>
          </div>
          <p className="text-[11px] text-slate-500 leading-relaxed">
            Your assets are protected by the blockchain.
          </p>
        </div>
      )}
      ...
      <div className="px-3 mb-4">
        <div className={`flex justify-center transition-all ${collapsed ? 'scale-75' : 'scale-90'}`}>
          <WalletMultiButton />
        </div>
      </div>
      ...
      <div className="px-3 mb-4">
        <button
          onClick={() => {
            // This should trigger a logout in both useWallet and useAuth
            window.dispatchEvent(new CustomEvent('legacyvault_logout'));
          }}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition-all group"
        >
          <div className="w-9 h-9 rounded-lg bg-rose-500/10 flex items-center justify-center group-hover:bg-rose-500/20">
            <X className="w-4.5 h-4.5" />
          </div>
          {!collapsed && <span className="text-sm font-medium animate-fade-in">Log Out</span>}
        </button>
      </div>
