...
import type { DocumentView, DocumentType } from '../types/api';
...
const DocumentsPanel: FC<DocumentsPanelProps> = ({ documents, onUpload }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      onUpload(file, 'other'); // Defaulting to other for simple UI
    }
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-card rounded-2xl p-5">
          ...
          <p className="text-2xl font-bold text-white font-mono">{formatSize(totalSizeBytes)}</p>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {doc.encrypted && <Lock className="w-3 h-3 text-emerald-400" />}
              </div>
              <div className="flex items-center gap-3 mt-1">
                <span className="text-[10px] text-slate-500">{formatSize(doc.sizeBytes)}</span>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="p-2 rounded-lg hover:bg-white/5 text-slate-500 hover:text-vault-400 transition-colors"
              >
